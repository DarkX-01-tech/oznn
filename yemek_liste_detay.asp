...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    #header { background: linear-gradient(90deg, <% If menu_tipi = "diyet" Then %>var(--diyet), var(--diyet-dark)<% Else %>var(--main), var(--dark)<% End If %>); padding: 8px 15px; color: var(--txt); display: flex; align-items: center; justify-content: space-between; box-shadow: 0 4px 10px rgba(0,0,0,0.15); }
    ...
    .nav-button:hover { background: <% If menu_tipi = "diyet" Then %>var(--diyet-dark)<% Else %>var(--dark)<% End If %>; transform: translateY(-2px); }
    ...
    .ay-card { background: #fff; border-radius: 16px; box-shadow: 0 10px 40px <% If menu_tipi = "diyet" Then %>rgba(76,175,80,0.15)<% Else %>rgba(69,184,195,0.15)<% End If %>, 0 4px 12px rgba(0,0,0,0.06); overflow: hidden; width: 540px; animation: cardIn 0.5s cubic-bezier(0.22,1,0.36,1); }
    @keyframes cardIn { from { opacity: 0; transform: translateY(20px) scale(0.96); } to { opacity: 1; transform: translateY(0) scale(1); } }

    .ay-card-header { background: linear-gradient(135deg, <% If menu_tipi = "diyet" Then %>var(--diyet), var(--diyet-dark)<% Else %>var(--main), var(--dark)<% End If %>); color: #fff; padding: 28px; text-align: center; position: relative; overflow: hidden; }
    ...
    .stat-row { display: flex; align-items: center; gap: 14px; background: linear-gradient(135deg, <% If menu_tipi = "diyet" Then %>rgba(76,175,80,0.08), rgba(56,142,60,0.03)<% Else %>rgba(69,184,195,0.08), rgba(46,139,145,0.03)<% End If %>); border-left: 3px solid <% If menu_tipi = "diyet" Then %>var(--diyet)<% Else %>var(--main)<% End If %>; border-radius: 8px; padding: 16px; margin-bottom: 22px; }
    .stat-icon { background: linear-gradient(135deg, <% If menu_tipi = "diyet" Then %>var(--diyet), var(--diyet-dark)<% Else %>var(--main), var(--dark)<% End If %>); width: 44px; height: 44px; border-radius: 12px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
    ...
    .btn-act::before { content: ''; position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: linear-gradient(180deg, rgba(255,255,255,0.15) 0%, rgba(255,255,255,0) 100%); pointer-events: none; }
    .btn-act svg { width: 24px; height: 24px; flex-shrink: 0; position: relative; z-index: 1; }
    .btn-act span { position: relative; z-index: 1; line-height: 1.3; }
    .btn-act:hover { transform: translateY(-3px); }
    .btn-act:active { transform: translateY(0); }

    .btn-toplu { background: linear-gradient(135deg, <% If menu_tipi = "diyet" Then %>var(--diyet), var(--diyet-dark)<% Else %>var(--main), var(--dark)<% End If %>); color: #fff; border-color: <% If menu_tipi = "diyet" Then %>var(--diyet)<% Else %>var(--main)<% End If %>; }
    .btn-toplu:hover { box-shadow: 0 8px 24px <% If menu_tipi = "diyet" Then %>rgba(76,175,80,0.4)<% Else %>rgba(69,184,195,0.4)<% End If %>; }

    .btn-tekil { background: #fff; color: <% If menu_tipi = "diyet" Then %>var(--diyet-dark)<% Else %>var(--dark)<% End If %>; border-color: <% If menu_tipi = "diyet" Then %>var(--diyet)<% Else %>var(--main)<% End If %>; }
    .btn-tekil::before { display: none; }
    .btn-tekil:hover { background: linear-gradient(135deg, <% If menu_tipi = "diyet" Then %>var(--diyet), var(--diyet-dark)<% Else %>var(--main), var(--dark)<% End If %>); color: #fff; box-shadow: 0 8px 24px <% If menu_tipi = "diyet" Then %>rgba(76,175,80,0.4)<% Else %>rgba(69,184,195,0.4)<% End If %>; }

    .btn-sil { background: #fff; color: #e53935; border-color: #ef9a9a; }
    .btn-sil::before { display: none; }
    ...
    .btn-geri-act::before { display: none; }
    ...
    .log-toggle:hover { padding-right: 14px; background: linear-gradient(180deg, #455a64, #263238); box-shadow: -4px 0 16px rgba(0,0,0,0.2); }
...
<body>
  <div id="header">
    ...
    <h1>
      ...
      <%= secilen_yil %>
      <% If menu_tipi = "diyet" Then %>
      <span class="header-badge">Diyet</span>
      <% End If %>
    </h1>
  ...
  <div class="page-wrapper">
    <div class="ay-card">
      <div class="ay-card-header">
        ...
        <% If menu_tipi = "diyet" Then %>
        <span class="type-label">Diyet Men&#252;s&#252;</span>
        <% End If %>
      </div>
  ...
  <div class="log-panel" id="logPanel">
    ...
    <div class="log-body">
      ...
      <div class="log-empty">
        ...
        <p>Log tablosu hen&#252;z olu&#351;turulmam&#305;&#351;.<br><strong>yemek_guncelleme_log</strong> tablosunu ekleyin.</p>
      </div>
